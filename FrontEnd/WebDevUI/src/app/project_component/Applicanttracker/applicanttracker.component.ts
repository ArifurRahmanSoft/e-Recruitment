...
@Component({
  selector: 'app-applicanttracker',
  templateUrl: './applicanttracker.component.html',
  styleUrls: ['./applicanttracker.component.scss'],
...
public registerForm:any;
  ...
  createRecruitmentForm() {
    this.recruitmentApprovalForm = new FormGroup({
      ...
      salaryReviewer: new FormControl(null, Validators.required),
      ...
      hrBP: new FormControl(null, Validators.required),
      headOfUnit: new FormControl(null, Validators.required),
      headOfBusiness: new FormControl(null, Validators.required),
      finalAprvHR: new FormControl(null, Validators.required),
  ...
  updateRecXm(rexXm: any[]) {
    ...
    rexXm.forEach(xm => {
      ...
      setTimeout(() => {
        // let valuesToSet = [];
        // if (xm.examinarIdLst && Array.isArray(xm.examinarIdLst)) {
        //   valuesToSet = xm.examinarIdLst;
        // } else if (typeof xm.examinarIdLst === 'string' && xm.examinarIdLst.includes(',')) {
        //   valuesToSet = xm.examinarIdLst.split(',').map(id => id.trim());
        // } else if (typeof xm.examinarIdLst === 'string') {
        //   valuesToSet = [xm.examinarIdLst];
        // }
        // xmGroup.get('examinarIdLst')?.setValue(valuesToSet);
  ...
  getListByPage(pageSize) {
    debugger
    setTimeout(() => {
      this._pg.getListByPage(this.currentPage, true, pageSize, '');
      setTimeout(() => {
      }, 300);
    }, 0);
  }
  ...
  getRequsitionById(list: any) {
    ...
    this._dataservice.getWithMultipleModel(apiUrl, param)
      .subscribe(response => {
        ...
        this.isApproveNote = this.applicationMstr.approveDeclineNote;
        ...
        console.log("applicationMstr applicationMstr ", this.applicationMstr)
        this.clearRecExam();
...
public apiUrl:string='reqform/testsaveupdates'
saveData(){
  debugger
  var regValue=this.registerForm.value;
  var param=[regValue,'08427']
  this._dataservice.insertUser(param,this.apiUrl).subscribe({
    next:(response:any)=>{
      console.log(response)
      alert('this is successfully')
    },
    error:(error:any)=>{
      alert("somethisng ia wrong")
    }
  })
