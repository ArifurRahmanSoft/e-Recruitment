...
@Component({
  selector: 'app-approval-role',
  templateUrl: './approval-role.component.html',
  styleUrls: ['./approval-role.component.scss'],
  providers: [PagerService]
})
export class ApprovalRoleComponent implements OnInit {
  //Common
      ...
      .subscribe(
        response => {
          ...
          if (this.res.resdata.listUsrRole.length > 0) {
            ...
            this.roleList = list;
            console.log("this.roleList is ",this.roleList )
          }
        }, error => {
          console.log(error);
        });
  ...
  public _getUrl: string = 'jobuserRole/getapprovalbypage';
  getUserRoleByPage(pageIndex: number, isPaging: boolean, pageSize: number, companyId) {
    ...
    this._dataservice.getWithMultipleModel(apiUrl, param)
      .subscribe(
        response => {
          this.res = response;
          console.log("approval role is ",this.res)
          this.userRoleList = [];

          if (this.res.resdata.listApprovalRole.length > 0) {
            this.userRoleList = JSON.parse(this.res.resdata.listApprovalRole);
  ...
  public _saveUrl: string = 'jobuserRole/saveUpdateApproval';
  onSubmit() {
    debugger
  ...
  //Get by ID
  public _getbyIdUrl: string = 'jobuserRole/getapprovalbyid';
  edit(modelEvnt) {
    debugger
